  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        <div
          className={cn(
            "text-[14px] text-right min-w-[140px] tabular-nums text-text-primary",
      ...
      {hasChildren && isOpen ? (
        <div className="flex flex-col">
          {line.children?.map((child, index) => (
            <ProfitAndLossRow key={child.id || `profit-loss-${depth}-${index}`} line={child} depth={depth + 1} />
          ))}
        </div>
      ) : null}
    </div>
      ...
      <div className="max-w-4xl mx-auto card-base p-10 min-h-[1000px] flex flex-col rounded-sm">
        ...
        <div className="flex-1 py-4">
          {Array.isArray(data) && data.length > 0 ? (
            data.map((category, index) => (
              <ProfitAndLossRow key={category.id || `profit-loss-category-${index}`} line={category} depth={0} />
            ))
          ) : (
        ...
        <div className="mt-16 pt-8 border-t border-border flex flex-col items-center gap-4">
          ...
          <p className="text-[11px] text-text-muted text-center max-w-sm leading-relaxed">
            This report provides a granular view of the company&apos;s financial performance.
          </p>
        </div>
      </div>
    </div>
